...
export default async function CashDetailPage({
  ...
  if (!session) notFound();
  const income = session.transactions
    .filter((transaction) => transaction.type === "INCOME")
    .reduce((sum, transaction) => sum + Number(transaction.amount), 0);
  const expense = session.transactions
  ...
  return (
    <main className="admin-page printable-report">
      ...
      <section className="data-panel">
        <p>
          <strong>Caixa:</strong> {session.account.name}
        </p>
        <p>
          <strong>Aberto por:</strong> {session.openedBy.name} em{" "}
          {session.openedAt.toLocaleString("pt-BR")}
        </p>
        <p>
          <strong>Valor inicial:</strong>{" "}
          {format(Number(session.openingBalance))}
        </p>
        <div className="metric-grid">
          ...
          <article>
            <span>Saídas</span>
            <strong>{format(expense)}</strong>
          </article>
          <article>
            <span>Valor esperado</span>
            <strong>
              {format(Number(session.expectedBalance ?? expected))}
            </strong>
          </article>
          <article>
            ...
            <strong>
              {session.closingCounted === null
                ? "Em aberto"
                : format(Number(session.closingCounted))}
            </strong>
          </article>
        </div>
        <p
          ...
          <strong>
            {session.difference === null
              ...
              : Number(session.difference) < 0
                ? `FALTA DE CAIXA: ${format(Math.abs(Number(session.difference)))}`
                : Number(session.difference) > 0
                  ? `SOBRA DE CAIXA: ${format(Number(session.difference))}`
                  : "Diferença: R$ 0,00"}
          </strong>
        </p>
        {session.closedBy && (
          <p>
            <strong>Fechado por:</strong> {session.closedBy.name} em{" "}
            {session.closedAt?.toLocaleString("pt-BR")}
          </p>
        )}
        <h2>Movimentações</h2>
            ...
            <tbody>
              {session.transactions.map((transaction) => (
                <tr key={transaction.id}>
                  <td>{transaction.occurredAt.toLocaleString("pt-BR")}</td>
                  <td>{transaction.type === "INCOME" ? "Entrada" : "Saída"}</td>
                  <td>{transaction.description}</td>
                  <td>{transaction.category?.name ?? "-"}</td>
                  <td>{format(Number(transaction.amount))}</td>
