...
export default async function DashboardPage() {
  ...
  const [income, expense, pending, recent, cashSession] = await Promise.all([
  ...
  const currentCash = cashSession
    ? Number(cashSession.openingBalance) + recent
        .filter((transaction) => transaction.cashSessionId === cashSession.id)
    ...
    <main className="dashboard">
      <header className="dashboard-header">
        ...
        <form action="/api/auth/logout" method="post">
          <button className="switch-button" type="submit">Sair</button>
        </form>
      </header>
      ...
      <section className="activity">
        ...
        {recent.length === 0 ? (
          <div className="empty-state-card">
            ...
            <p>Registre sua primeira movimentação para manter o caixa e o financeiro organizados.</p>
          </div>
        ) : (
            ...
            {recent.map((transaction) => (
              <div className="transaction-row" key={transaction.id}>
                <div className="transaction-info">
                  ...
                  <small>
                    {transaction.occurredAt.toLocaleDateString("pt-BR")} • {transaction.category?.name ?? "Sem categoria"} • {transaction.account?.name ?? "Sem conta"}
                  </small>
                </div>
                <div className="transaction-amount">
                  <span className={transaction.type === "INCOME" ? "status-badge success" : "status-badge danger"}>
