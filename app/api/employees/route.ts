...
export async function GET() {
  ...
  return NextResponse.json(await prisma.employee.findMany({ where: { restaurantId: auth.ctx.restaurant.id }, orderBy: { name: "asc" } }));
}
...
export async function POST(request: Request) {
  ...
  const row = await prisma.employee.create({ data: { ...parsed.data, monthlySalary: parsed.data.monthlySalary?.toFixed(2), restaurantId: auth.ctx.restaurant.id } });
  await audit({ restaurantId: auth.ctx.restaurant.id, actorUserId: auth.ctx.user.id, action: "CREATE", entity: "Employee", entityId: row.id, data: parsed.data });
