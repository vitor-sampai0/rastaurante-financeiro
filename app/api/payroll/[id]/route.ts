...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params; const parsed = schema.safeParse(await request.json().catch(() => null)); if (!parsed.success) return jsonError("Dados inválidos");
  ...
  const account = await prisma.account.findFirst({ where: { id: parsed.data.accountId, restaurantId: auth.ctx.restaurant.id } }); if (!account) return jsonError("Conta inválida", 400);
