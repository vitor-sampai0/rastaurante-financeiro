...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (payment.success) {
    ...
    const account = await prisma.account.findFirst({ where: { id: payment.data.accountId, restaurantId: auth.ctx.restaurant.id, active: true } });
    if (!account) return jsonError("Conta inválida", 400);
  ...
  const parsed = editSchema.safeParse(body); if (!parsed.success) return jsonError("Dados inválidos"); if (current.status === "PAID") return jsonError("Conta paga não pode ser editada", 409); if (!(await refsBelongToRestaurant(auth.ctx.restaurant.id, parsed.data))) return jsonError("Referência inválida", 400); const data = { ...parsed.data, amount: parsed.data.amount?.toFixed(2) }; const row = await prisma.accountsPayable.update({ where: { id }, data }); await audit({ restaurantId: auth.ctx.restaurant.id, actorUserId: auth.ctx.user.id, action: "UPDATE", entity: "AccountsPayable", entityId: id, data: parsed.data }); return NextResponse.json(decimalToNumber(row));
}
